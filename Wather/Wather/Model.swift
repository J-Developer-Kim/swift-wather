...
class WeatherDataSource {
    ...
    func fetchSummary(lat: Double, lon: Double, completion: @escaping ()->()) {
        let apiUrl = "https://api2.sktelecom.com/weather/current/minutely?version=1&lat=\(lat)&lon=\(lon)&appKey=\(Comment.appKey)"
    ...
    func fetchForecast(lat: Double, lon: Double, completion: @escaping ()->()) {
        ...
        let apiUrl = "https://api2.sktelecom.com/weather/forecast/3days?version=1&lat=\(lat)&lon=\(lon)&appKey=\(Comment.appKey)"
        ...
        let task = session.dataTask(with: url) { (data, response, error) in
            ...
            do {
                let decoder = JSONDecoder()
                let forecase = try decoder.decode(Forecast.self, from: data)
                
                if let list = forecase.weather.forecast3days.first?.fcst3hour.arrayRepresentation() {
